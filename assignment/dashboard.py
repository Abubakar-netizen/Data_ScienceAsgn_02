...
def main():
    ...
    with tab1:
        ...
        if not df_filtered.empty:
            ...
            col2.metric("Date Range", f"{df_filtered['date'].min().date()} - {df_filtered['date'].max().date()}")
        ...
        **Methodology**:
        ...
        if st.button("Run Dimensionality Analysis"):
            if df_filtered.empty:
                st.warning("Cannot run analysis on an empty dataset. Please adjust filters.")
            else:
                with st.spinner("Processing High-Dimensional Data..."):
                    pca_df, loadings, report = pca_analyzer.run_analysis(df_filtered)
                    
                    if pca_df.empty:
                        st.error("PCA analysis failed to produce results. Data might be insufficient after cleaning.")
                    else:
                        col_a, col_b = st.columns([2, 1])
                        with col_a:
                            st.plotly_chart(plot_pca_scatter(pca_df), width="stretch")
                        with col_b:
                            st.write("### Variance Explained")
                            st.bar_chart(report['explained_variance'])
                            st.write("### PCA Loadings Analysis")
                            st.plotly_chart(plot_pca_loadings(loadings), width="stretch")
    ...
    with tab3:
        ...
        if df_filtered.empty:
            ...
            st.plotly_chart(plot_high_density_heatmap(matrix, metric), width="stretch")
    ...
    with tab4:
        ...
        if df_filtered.empty:
            st.warning("No data available for distribution modeling.")
        else:
            target_col = "pm25"
            ...
            with col_viz1:
                st.plotly_chart(plot_distribution_peaks(df_filtered, target_col), width="stretch")
                st.markdown("*Optimized to reveal Peaks (Linear Scale)*")
            with col_viz2:
                ccdf_data = dist_analyzer.get_tail_data(df_filtered, target_col)
                st.plotly_chart(plot_distribution_tails(ccdf_data, target_col), width="stretch")
    ...
    with tab5:
        ...
        if df_filtered.empty:
            st.warning("No data available for visual audit implementation.")
        else:
            st.subheader("Implementation: Bivariate Mapping")
            st.plotly_chart(audit_engine.get_bivariate_mapping(df_filtered), width="stretch")
            
            st.subheader("Implementation: Small Multiples")
            st.plotly_chart(audit_engine.get_small_multiples(df_filtered), width="stretch")
